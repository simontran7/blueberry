...
use crate::core::common::handle_collections::handle_list::HandleRange;
use crate::core::common::symbol::Symbol;
use crate::core::semantic_analysis::definition_body_builder::DefinitionBodyBuilder;
...
impl<'db> DefinitionBodyLowerer<'db> {
        ...
        match expression {
            ...
            ast::Expression::PathExpression(path_expression) => {
                let lowered = path_expression
                    .path()
                    .and_then(|path| Path::lower(self.db, &path))
            ...
            ast::Expression::UnaryOperation(unary) => {
                ...
                let operand = self.lower_optional_expression(unary.operand(), unary.red());
            ...
            ast::Expression::BinaryOperation(binary) => {
                let lhs = self.lower_optional_expression(binary.lhs(), binary.red());

                let operator = binary
                    .operator()
                    .and_then(|token| BinaryOperator::try_from(token.kind()).ok());

                let rhs = self.lower_optional_expression(binary.rhs(), binary.red());
            ...
            ast::Expression::Call(call) => {
                let callee = self.lower_optional_expression(call.callee(), call.red());
                ...
                self.builder
                    .add_expression(Expression::Call { callee, arguments }, call.red())
            }
            ast::Expression::Assignment(assignment) => {
                let target = self.lower_optional_expression(assignment.target(), assignment.red());

                let value = self.lower_optional_expression(assignment.value(), assignment.red());

                self.builder
                    .add_expression(Expression::Assignment { target, value }, assignment.red())
            }
            ast::Expression::Return(return_expression) => {
                ...
                self.builder
                    .add_expression(Expression::Return { value }, return_expression.red())
            }
            ast::Expression::Break(break_expression) => {
                let value = break_expression
                    .value()
                    .map(|value| self.lower_expression(value));

                self.builder
                    .add_expression(Expression::Break { value }, break_expression.red())
            }
            ...
            ast::Expression::InfiniteLoop(infinite_loop) => {
                let body = self.lower_optional_block(infinite_loop.body(), infinite_loop.red());
                self.builder.add_expression(
                    Expression::Loop {
    ...
    fn lower_while_loop(&mut self, while_loop: ast::WhileLoop) -> ExpressionHandle {
        ...
        let condition = self.lower_optional_expression(while_loop.condition(), node);
        let then_branch = self.lower_optional_block(while_loop.body(), node);
        let break_expression = self
            .builder
            .add_expression(Expression::Break { value: None }, node);
        let if_expression = self.builder.add_expression(
            Expression::If {
                condition,
                then_branch,
                else_branch: Some(break_expression),
            },
            node,
        );
        let statements = self.builder.add_statement_children(&[]);
        let body = self.builder.add_expression(
            Expression::Block {
                key: None,
                statements,
                tail: Some(if_expression),
            },
            node,
        );
        self.builder.add_expression(
        ...
        match statement {
            ast::Statement::LetStatement(let_statement) => {
                let name = let_statement
                    .name()
                    .map_or_else(String::new, |token| token.lexeme().to_string());
                let name = self.builder.add_local_binding(
                    LocalBinding {
                        name: Symbol::new(self.db, name),
            ...
            ast::Statement::ExpressionStatement(expression_statement) => {
                let expression = self.lower_optional_expression(
