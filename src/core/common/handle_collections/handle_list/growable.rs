...
/// A 4-byte handle to a growable, mutable list of `H`s living in a [`GrowableHandleListAllocator`].
...
///
/// `start` is the index in the allocator's backing storage `GrowableHandleListAllocator::data` where
...
/// # Safety
///
/// [`GrowableHandleList`] is `Copy`, but must be treated as a unique logical owner of its
/// allocated block. Aliasing copies (e.g. cloning a handle and calling [`GrowableHandleList::clear`]
...
///
/// ```rust,ignore
/// let mut allocator = GrowableHandleListAllocator::<ValueId>::new();
/// let mut list = GrowableHandleList::<ValueId>::from(&mut allocator, &[ValueId::new(1), ValueId::new(2)]);
/// list.add_last(&mut allocator, ValueId::new(3));
/// assert_eq!(list.to_slice(&allocator), &[ValueId::new(1), ValueId::new(2), ValueId::new(3)]);
/// ```
#[derive(Clone, Copy)]
pub(crate) struct GrowableHandleList<H> {
    pub(super) start: u32,
    _marker: PhantomData<H>,
}

impl<H> fmt::Debug for GrowableHandleList<H> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("GrowableHandleList")
            .field("start", &self.start)
            .finish()
    }
}

impl<H> Default for GrowableHandleList<H> {
    fn default() -> Self {
        Self {
            start: 0,
            _marker: PhantomData,
...
/// A Segregated Free List suballocator storing every [`GrowableHandleList`]'s content.
///
/// # Layout
///
...
/// [`GrowableHandleList`]'s documentation).
...
pub(crate) struct GrowableHandleListAllocator<H> {
...
impl<H: Handle> GrowableHandleList<H> {
    /// Marks an empty list.
    ///
    ...
    pub(crate) fn from(allocator: &mut GrowableHandleListAllocator<H>, slice: &[H]) -> Self {
    ...
    pub(crate) fn count(self, allocator: &GrowableHandleListAllocator<H>) -> usize {
    ...
    pub(crate) fn is_empty(self, allocator: &GrowableHandleListAllocator<H>) -> bool {
        self.count(allocator) == 0
    }

    /// Returns the list's elements as a slice, or an empty slice if empty.
    pub(crate) fn to_slice(self, allocator: &GrowableHandleListAllocator<H>) -> &[H] {
    ...
    pub(crate) fn to_mut_slice(self, allocator: &mut GrowableHandleListAllocator<H>) -> &mut [H] {
        if self.is_empty(allocator) {
            &mut []
        } else {
    ...
    pub(crate) fn get(&self, allocator: &GrowableHandleListAllocator<H>, index: usize) -> Option<H> {
        self.to_slice(allocator).get(index).copied()
    }

    /// Adds `value` to the end of the list.
    pub(crate) fn add_last(&mut self, allocator: &mut GrowableHandleListAllocator<H>, value: H) {
        let start = self.start as usize;
        if self.is_empty(allocator) {
    ...
    pub(crate) fn remove(&mut self, index: usize, allocator: &mut GrowableHandleListAllocator<H>) {
        let count = self.count(allocator);
    ...
    pub(crate) fn clear_last(&mut self, allocator: &mut GrowableHandleListAllocator<H>) {
    ...
    pub(crate) fn clear(&mut self, allocator: &mut GrowableHandleListAllocator<H>) {
...
impl<H: Handle> GrowableHandleListAllocator<H> {
    ...
    /// Drops every [`GrowableHandleList`]'s contents and free-list state, returning the allocator to its initial empty state.
    pub(crate) fn reset(&mut self) {
        // Any `GrowableHandleList` into this allocator is invalidated.
        self.data.clear();
        self.free.clear();
    }
...
mod tests {
    ...
    crate::core::common::handle_collections::handle_impl!(pub(crate) TestId);
    ...
    fn new_list_is_empty() {
        let allocator = GrowableHandleListAllocator::<TestId>::new();
        let list = GrowableHandleList::<TestId>::new();
    ...
    fn from_empty_slice_is_empty_without_allocating() {
        let mut allocator = GrowableHandleListAllocator::<TestId>::new();
        let list = GrowableHandleList::<TestId>::from(&mut allocator, &[]);
        assert!(list.is_empty(&allocator));
    ...
    fn from_slice_copies_elements() {
        ...
        let list = GrowableHandleList::<TestId>::from(&mut allocator, &values);
    ...
    fn add_last_grows_across_size_class_boundaries() {
        let mut allocator = GrowableHandleListAllocator::<TestId>::new();
        let mut list = GrowableHandleList::<TestId>::new();
    ...
    fn get_returns_none_out_of_bounds() {
        let mut allocator = GrowableHandleListAllocator::<TestId>::new();
        let list = GrowableHandleList::<TestId>::from(&mut allocator, &ids(&[10, 20]));
    ...
    fn to_mut_slice_allows_in_place_mutation() {
        let mut allocator = GrowableHandleListAllocator::<TestId>::new();
        let list = GrowableHandleList::<TestId>::from(&mut allocator, &ids(&[10, 20, 30]));
    ...
    fn remove_shifts_later_elements_and_preserves_order() {
        let mut allocator = GrowableHandleListAllocator::<TestId>::new();
        let mut list = GrowableHandleList::<TestId>::from(&mut allocator, &ids(&[0, 1, 2, 3]));
    ...
    fn remove_panics_out_of_bounds() {
        let mut allocator = GrowableHandleListAllocator::<TestId>::new();
        let mut list = GrowableHandleList::<TestId>::from(&mut allocator, &ids(&[0, 1]));
    ...
    fn clear_last_shrinks_from_the_end() {
        let mut allocator = GrowableHandleListAllocator::<TestId>::new();
        let mut list = GrowableHandleList::<TestId>::from(&mut allocator, &ids(&[0, 1, 2]));
    ...
    fn clear_frees_the_block_for_reuse() {
        let mut allocator = GrowableHandleListAllocator::<TestId>::new();
        let mut a = GrowableHandleList::<TestId>::from(&mut allocator, &ids(&[0, 1, 2]));
        ...
        let b = GrowableHandleList::<TestId>::from(&mut allocator, &ids(&[9, 8, 7]));
